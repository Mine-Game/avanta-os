  ...
  return `
    <div class="pm-modal-backdrop" id="pmTaskModalBackdrop">
      <div class="pm-modal pm-modal-luxe" role="dialog" aria-modal="true">
        ...
        <div class="pm-modal-section">
          <small>NOTES</small>
          <div class="pm-note-box">${blockers.length ? `⚠ ${escapeHtml(blockers[0].reason || 'Blocked')}` : 'Task is in progress. No blockers right now.'}</div>
        </div>
        ...
        <div class="pm-modal-section">
          ...
          <div class="pm-deliverables-list">
            ${deliverables.length
              ? deliverables
                  .map(
                    (d) => `<div class="pm-deliverable-row"><span>• ${escapeHtml(d.deliverableName || 'Deliverable')}</span><span class="pm-chip">${escapeHtml(String(d.status || 'planned').replace(/_/g, '-'))}</span></div>`
                  )
                  .join('')
...
function renderOpsBoard() {
  ...
  const stageMeta = {
    intake: { label: 'To Do', icon: '⚪', tone: 'todo' },
    scoping: { label: 'Scoping', icon: '🟦', tone: 'scoping' },
    activation: { label: 'In Progress', icon: '🟡', tone: 'progress' },
    review: { label: 'Review', icon: '🟠', tone: 'review' },
    execution: { label: 'On Hold', icon: '🟣', tone: 'hold' },
    update: { label: 'Done', icon: '🟢', tone: 'done' },
  };
                        ...
                        .map((t) => {
                          ...
                          return `
                            <article class="pm-luxe-task" draggable="true" data-pm-task-id="${t.id}" data-pm-stage="${stage}" data-open-pm-task="${t.id}">
                              ...
                              ${isBlocked ? `<div class="pm-task-alert">⚠ ${escapeHtml((t.blockers || [])[0]?.reason || 'Blocked')}</div>` : ''}
...
function renderOpsStandups() {
  ...
  return `
    <section class="standups-v2-shell">
      ...
      <div class="standups-v2-note">Каркас без backend: записи и воспроизведение подключим на следующей неделе.</div>
      ${renderStandupModal()}
    </section>
  `;
}
                ...
                .map((job) => {
                  ...
                  return `
                    <tr class="automation-row-main ${isOpen ? 'open' : ''}">
                      <td><button class="ghost-btn automation-expand-btn" data-automation-toggle-id="${escapeHtml(job.id || '')}">${isOpen ? '−' : '+'}</button></td>
